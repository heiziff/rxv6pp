...
*/
uint64 print_pt(pagetable_t pagetable, int lvl) {
  if (lvl == 0) { return 0; }
  union debugpte_t {
    pte_t table;
    struct {
  ...
  for (int i = 0; i < 512; i++) {
    ...
    if (*pte & PTE_V) {
      ...
      for (int i = 3; i >= lvl; i--) { pr_notice("%d:", i); }
      pr_info("num:%d: %p, Flags: V:%d, R:%d, W:%d, X:%d, U:%d, \n", i, entry.table,
        entry.debug.bflags.valid, entry.debug.bflags.read, entry.debug.bflags.write,
        entry.debug.bflags.execute, entry.debug.bflags.user);
