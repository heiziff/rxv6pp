...
static void printnum(long xx, int base, int sign) {
...
void setoutputpriority(char level) {
  ...
  char c;
  for (int i = 0; (c = levelColorMap[colorIndex][i] & 0xff) != 0; i++) { consputc(c); }
}
...
void printk(char *fmt, ...) {
  ...
  for (i = 1; (c = fmt[i] & 0xff) != 0; i++) {
    ...
    if (c == 0) break;
    switch (c) {
    case 'd': printnum(va_arg(ap, int), 10, 1); break;
    case 'x': printnum(va_arg(ap, int), 16, 1); break;
