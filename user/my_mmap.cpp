...
int main(int argc, char **) {
  ...
  for (int i = 0; i < 3; i++) { test(0, 1, PROT_RW, MAP_PRIVATE); }
  for (int i = 0; i < 3; i++) { test((void *)((uint64)1 << 36), 1, PROT_RW, MAP_PRIVATE); }
  for (int i = 0; i < 3; i++) {
  ...
  for (int i = 0; i < 5; i++) { test((void *)addr, i + 1, PROT_RW, MAP_PRIVATE | MAP_FIXED); }
  for (int i = 0; i < 5; i++) { munmap((void *)addr, i + 1); }
